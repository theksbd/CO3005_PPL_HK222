...
class ASTGeneration(MT22Visitor):
    ...
    def visitVardeclAssignBaseCase(self, ctx: MT22Parser.VardeclAssignBaseCaseContext):
        identifier = ctx.IDENTIFIER().getText()
        typ = self.visit(ctx.typ()) if ctx.typ(
    ...
    def visitFuncdecl(self, ctx: MT22Parser.FuncdeclContext):
        ...
        inheritanceSubpart = self.visit(
            ctx.inheritanceSubpart()) if ctx.inheritanceSubpart() else None
    ...
    def visitExpr4(self, ctx: MT22Parser.Expr4Context):
        ...
        op = ctx.MUL().getText() if ctx.MUL() else (
            ctx.DIV().getText() if ctx.DIV() else ctx.MODUL().getText())
        expression4 = self.visit(ctx.expr4())
